...
- **HIGH — FIXED**: ~~`USE_TZ = False` (line 185) directly contradicts this project's own written rule ("Use timezone-aware datetimes") and is visibly inconsistent with code that already assumes timezone awareness (`accounts/signals.py`'s `timezone.activate()` call, which is largely inert under `USE_TZ=False`).~~ Set `USE_TZ = True`. This surfaced three naive-`datetime.now()` call sites that were writing naive datetimes into `DateTimeField`s (`accounts/models.py`'s `User.add_login`, `accounts/admin.py`'s `all_unexpired_sessions_for_user`, `content/signals.py`'s `verify_news_article`); all three were switched to `django.utils.timezone.now()`. Remaining known risk: `sitesettings/serializers.py:70` (`SNSSubscriptionRequestSerializer.create`) still builds a naive datetime via `datetime.strptime(...)` for `SNSSubscriptionRequest.timestamp`, and `accounts/fixtures/test_users.json` contains naive `date_joined` values — both now emit `RuntimeWarning` under `USE_TZ=True` (not test failures) and are candidates for a follow-up fix, out of scope for this change.
- **HIGH — FIXED**: ~~`EMAIL_BACKEND = 'django_ses.SESBackend'` (line 217), but `django-ses` is **not** in `requirements.txt` and is not installed in this environment (confirmed via `pip show django-ses`) — any code path that sends email (confirmation codes, password resets, welcome emails) will raise `ModuleNotFoundError` in an environment built strictly from `requirements.txt`.~~ Added `django-ses==4.7.2` to `requirements.txt` and installed it, so the configured backend now actually resolves (`from django_ses import SESBackend` succeeds; no `INSTALLED_APPS` entry required for the backend itself to work).
- **FIXED**: ~~`CORS_ORIGIN_ALLOW_ALL = True` / `CORS_ALLOW_ALL_ORIGINS = True` were set with `django-cors-headers` not installed~~ — the two dead, dangerously-permissive lines were removed from `settings.py`. No CORS package is installed or needed today; if cross-origin API access is required later, `CORS_ALLOWED_ORIGINS` should be set explicitly from an env-driven list, not `ALLOW_ALL`.
- **FIXED (partially)**: ~~`REST_FRAMEWORK['PAGINATE_BY'] = 2` was a DRF 2.x-era setting name with no effect~~ — the dead key was removed from `settings.py` (confirmed nothing else in the codebase referenced it). Real pagination (`DEFAULT_PAGINATION_CLASS`/`PAGE_SIZE`) was deliberately **not** added in this pass: many `ListAPIView`s in this codebase wrap `response.data` assuming it's a plain list (e.g. `Response({'buddies': response.data})`), and DRF pagination changes `response.data` to a `{count, next, previous, results}` object — enabling it globally would silently break every one of those response shapes. Turning on real pagination is a separate, larger decision that needs an audit of each affected view.
- **FIXED**: ~~`openai==2.15.0` was pinned in `requirements.txt` but never imported anywhere~~ — removed from `requirements.txt`.
