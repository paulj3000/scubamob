...
- [ ] Triage and fix remaining CODE_REVIEW.md findings on `main`, starting with the other CRITICAL cross-app items in its §2 (unauthenticated `/tmp`-writing webhooks + stored XSS chain, weak password policy, no-op `SetPasswordApi`/`SetUsernameApi`, image upload pipeline).
