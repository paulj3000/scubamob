...
8. [x] Replace `SettingsApi`/`SystemApi.get_settings_server` call sites — done 2026-08-11. Unlike items 6/7, all three live endpoints (`GET_USER_SETTINGS_WITH_OPTIONS` = `/api/users/:userid/options`, `POST_USER_SETTINGS`/`GET_USER_SETTING_LIST` both = `/api/users/:userid`) had real fixture values, so no phantom-endpoint judgment call was needed. Added `settings.SETTINGS_SERVER`. `UserSettingApi.get()`/`.post()` and `UserSettingListApi.get()` (`scuba/accounts/apis/settings.py`) now build their URLs inline from `SETTINGS_SERVER` instead of calling `SettingsApi.get_user_settings_with_options()`/`post_user_settings()`/`get_user_setting_list()`. `SettingsApi.get_add_user_setting()` was already dead (zero call sites anywhere, confirmed via grep, pre-existing and unrelated to this item) and is untouched; the three now-orphaned-but-correct methods this item did replace are left in `sitesettings/models.py`, same treatment as item 4/5/6's leftovers, dropped with the app in items 14-15. Updated `test_outbound_request_timeouts.py`'s `TestSettingsApisTimeouts` to patch the imported `SETTINGS_SERVER` name instead of the removed `SettingsApi` methods. Verified with `manage.py check` (clean), `makemigrations --check --dry-run` (no changes), full `pytest` (264 passed, 5 skipped, 0 failed — no test count change, same tests updated in place).
9. [x] Replace `AlertingApi`/`SystemApi.get_alerting_*` call sites — done 2026-08-11. Added `settings.ALERTING_SERVER`/`ALERT_SERVER_ACTIVE`. No `AskUserQuestion` needed, but found two real, fixable bugs while tracing the call chain (same "wrong table" pattern as item 5's `get_s3_delete`, not the "never configured anywhere" pattern from items 6/7): **`scuba/libs/alerting.py`**: `Alerting.send_buddy_request()` called `SystemApi.get_alerting_buddy_request()`, which looked up a `SystemApi` key (`'ALERTING_BUDDY_REQUEST'`) that never existed in that model's choices or any fixture — the *real*, fixture-configured value lived on the sibling `AlertingApi` model instead (key `'BUDDY_REQUEST'` = `/api/alerts/buddies/request`), just wired to the wrong table. Fixed by building the URL directly from `ALERTING_SERVER` + the now-hardcoded, known-correct relative path. **`scuba/accounts/apis/socket.py`**: `AlertsApi.get()`'s 200/400 response never actually depended on `SystemSetting.ALERT_SERVER_ACTIVE` at all (despite `test_api_alerts.py`'s tests being named as if it did) — it hinged entirely on whether a `SystemApi` row for the equally-never-configured key `'ALERTING_URL'` happened to exist, which is a separate, accidental gate. Rewrote `AlertsApi.get()` to actually check `settings.ALERT_SERVER_ACTIVE` first (return the intended 400 "alert server is offline" when off) and build the request URL from `ALERTING_SERVER` + the known `AlertingApi.ALERTS` path (`/api/alerts`) when on; also added the `timeout=5` this view's `requests.get()` was missing (not previously covered by the §3 item 14 timeout pass, which didn't list `socket.py`). `SocketApi.get()`'s `get_socket_server_settings()` loop (over `SOCKET_SERVER_ACTIVE`/`SOCKET_SERVER_URL`/`CHAT_SERVER` — only the last ever had a real fixture value) is now a direct `{'CHAT_SERVER': CHAT_SERVER}`, matching its actual current output rather than the two phantom keys it always silently dropped. Rewrote `test_api_alerts.py` to patch the imported `ALERT_SERVER_ACTIVE` name directly (`django.test.override_settings` doesn't affect `from scuba.settings import X`-style direct imports, since it patches `django.conf.settings`, not the importing module's own namespace — the first attempt at this test used `override_settings` and failed for exactly that reason) and dropped the redundant third case (`'false'` vs `0`) now that there's no string-parsing logic left to differentiate. Added `test_alerting.py` and `test_api_socket.py`, neither of which existed before. `AlertingApi` the model class, and `SystemApi`'s now-fully-orphaned `get_alerting_url`/`get_alerting_alerts`/`get_alerting_buddy_request`/`get_alert_notify_staff`/`get_alerting_endpoint`/`get_socket_server_settings` in `sitesettings/models.py`, are left in place per the item 4/5/6/8 precedent — dropped with the rest of the app in items 14-15. Verified with `manage.py check` (clean), `makemigrations --check --dry-run` (no changes), full `pytest`: 265 passed, 5 skipped, 0 failed (net +1: one redundant test dropped, two new test files added).
10. [x] Decide the fate of `SNSSubscriptionRequest`/webhook handling — done 2026-08-11: **kept**, not removed. The "unauthenticated/broken" finding this item's text refers to was already fixed in the 2026-08-06 session (`CODE_REVIEW.md` §2.3) — `CodeBuildAPI`/`CodePipelineAPI` (`scuba/aws/apis.py`) and `BouncedEmailsAPI` (`scuba/security/apis.py`) all verify the AWS SNS cryptographic signature via `scuba/libs/aws/sns.py`'s `verify_signature()` before processing anything, and have real, correct field-mapping logic for CodeBuild/CodePipeline's actual event shapes — this is working AWS-integration infrastructure, not a phantom microservice like items 6/7. Moved `SNSSubscriptionRequest` (the model) and `SNSSubscriptionRequestSerializer` out of `sitesettings` into `scuba.aws` (`models.py`/`serializers.py`), which already owned two of the three call sites and the sibling `CodeBuildJob`/`CodePipelineState` models it's used alongside; also moved `SNSSubscriptionRequestAdmin` (unchanged, including its pre-existing `print()`-instead-of-admin-message quirk in `confirm_sns_request` — a verbatim move, not a rewrite) into `scuba/aws/admin.py`. Neither `sitesettings` nor `aws` has migrations, so this was a pure code move — `db_table = 'sns_subscription_requests'` kept identical, zero data risk, verified via `makemigrations --check --dry-run` staying clean. Updated the two remaining import sites (`scuba/aws/apis.py`, `scuba/security/apis.py`) to import `SNSSubscriptionRequestSerializer` from `scuba.aws.serializers`. No test changes needed — `test_api_webhooks.py`/`test_blocked_emails.py` exercise everything through the live signed-payload HTTP flow, never importing the model/serializer by path. Verified with `manage.py check` (clean), `makemigrations --check --dry-run` (no changes), full `pytest` (265 passed, 5 skipped, 0 failed, no count change).
11. [x] Replace `SystemSetting.get_default_profile_image()` (`scuba/accounts/models.py`, two call sites) / `get_default_banner_image()` (`scuba/divesites/models.py`) call sites with settings-based defaults — done 2026-08-11. Simpler than items 5/6/9: `SystemSetting.get_default_profile_image()`/`get_default_banner_image()` fall back to `scuba.settings.PROFILE_BLANK_URL`/`BANNER_BLANK_URL` (already plain, non-env-var settings constants) whenever no `SystemSetting` row exists for `DEFAULT_PROFILE_IMAGE`/`DEFAULT_BANNER_IMAGE` — and per item 1's inventory, neither key has ever had a fixture row, so both call sites always resolved to those same constants anyway. No new setting needed: the two `accounts/models.py` call sites (`User.get_profile_image()` twice) now use `static(PROFILE_BLANK_URL)` directly, and `divesites/models.py`'s `Divesite`'s banner call site now returns `BANNER_BLANK_URL` directly. Removed the now-fully-unused `SystemSetting` import from both files. No test updates needed — neither call site had any existing test coverage. Done concurrently with an unrelated, larger rebuild of the chat feature happening in the same working tree/branch (`chats`) — touched only `scuba/accounts/models.py` and `scuba/divesites/models.py`, neither of which that rebuild is touching. Verified with `manage.py check` (clean), `makemigrations --check --dry-run` (no changes), `pytest` on `scuba/accounts/tests/` + `scuba/divesites/tests/` (163 passed, 3 skipped, 0 failed) and the full suite (256 passed, 5 skipped, 0 failed — lower total than the last sitesettings-item pass because the concurrent chat rebuild has deleted several chat test files, unrelated to this item).
12. [x] Update every test that references `sitesettings` fixtures/models directly — done 2026-08-11. Of the original 11-file list: `test_api_user_list.py` was deleted by the concurrent chat-feature rebuild (moot); `test_outbound_request_timeouts.py`/`test_weather.py`/`test_api_alerts.py` were already updated in items 4/6/8/9. The remaining 7 (`test_api_user_accounts.py`, `test_api_user_divesite_favorite.py`, `test_divesite_methods.py`, `test_api_user_divesites.py`, `test_api_search.py`, `test_api_home.py`, `test_context_processor_scuba.py`) all loaded `test_sitesettings.json` in their `fixtures = [...]` list, but none of their actual test bodies touch weather, maps, chat, alerting, settings-server, or AWS cloudfront — every code path that historically needed those DB rows was already migrated to settings-based config in items 4-11. Verified empirically rather than by inspection alone: removed the fixture from each file's list one at a time and re-ran that file's tests — all 7 passed unchanged with zero DB-config dependency, confirming the fixture load was already dead weight, not something to "update to a settings-based mock." Removed `"test_sitesettings.json"` from all 7. No test file anywhere in the project still references it (confirmed via grep) — the fixture itself is now fully orphaned, though the file is deliberately left in place until item 14 deletes the app and its fixtures together. Verified with `manage.py check` (clean), `makemigrations --check --dry-run` (no changes), full `pytest`: 290 passed, 5 skipped, 1 failed — the failure is the pre-existing, already-documented `test_generate_random_string` flake (Known Technical Debt, unrelated to this change), confirmed by re-running in isolation (6 passed). (`scuba/sitesettings/tests/test_apis.py` remains, to be deleted along with the app in item 14, not updated here.)
13. [x] Remove `'scuba.sitesettings'` from `INSTALLED_APPS` in `scuba/settings.py` — done 2026-08-11.
14. [x] Delete `scuba/sitesettings/` (models, admin, apis, serializers, exceptions, fixtures, templates) once nothing imports it — done 2026-08-11. Also removed the last two live call sites: `scuba/urls.py`'s import of `GetSystemSettingsApi`/`GetSystemEndpointsApi` and the three routes it served (`/api/endpoints`, `/api/sitesettings`, `/api/sitesettings/all`) — these were the config-store app's own self-serving REST endpoints (expose `SystemApi`/`Endpoint` rows), not consumed by anything else in the codebase, so they had no reason to survive the app's retirement. Also fixed `entrypoint.sh`, which `loaddata`'d `scuba/sitesettings/fixtures/site_settings.json` on every deploy — left in place, this would have broken every future deploy the moment the app and its fixtures were gone; removed that fixture from the `loaddata` call. Reworded two of my own test docstrings (`test_api_alerts.py`, `test_alerting.py`) that referenced "sitesettings" by name in historical-context comments, so the plan's own exit criterion (`grep -rn "sitesettings" scuba/` returns nothing) is genuinely met, not just "no code depends on it."
15. [x] Drop the now-orphaned tables — done 2026-08-11, script only, not executed against a populated database. Added `scripts/sql/drop_sitesettings_tables.sql` (`DROP TABLE IF EXISTS`, child tables `endpoint`/`endpoint_param` before their `system_api` parent, portable to both SQLite and MySQL), dropping `system_api`, `endpoint`, `endpoint_param`, `system_setting`, `alerting_api`, `aws_api`, `billing_api`, `chat_api`, `settings_api`, `api_key` — not `flag_option`/`sns_subscription_requests`, which moved to `scuba.accounts`/`scuba.aws` in items 2/10 and are still live. Verified the script itself by recreating the old schema (with the real FK from `endpoint`/`endpoint_param` to `system_api`) in a throwaway SQLite file with `PRAGMA foreign_keys = ON` and confirming it drops all 10 tables cleanly in dependency order. **The actual local `db.sqlite3` has 0 tables — it was never migrated** (pytest-django uses its own separate, torn-down-per-run test database), so running the script against it was a real but no-op execution; there is no local data to lose. This script has **not** been run against any populated environment (staging/production MySQL, or any real deployed SQLite) — that remains a manual deploy step for whoever operates those environments, since neither `sitesettings` nor any other app in this project has migrations, so there is no automatic `migrate` path to do this.
16. [x] Run `python manage.py check`, `python manage.py makemigrations --check --dry-run`, and `pytest` to confirm nothing still imports `scuba.sitesettings` — done 2026-08-11, all clean (see Recently Completed for exact counts).

Exit criteria: met. `grep -rn "sitesettings" scuba/` returns nothing, all tests pass, and no live config value or the `FlagOption`/`SNSSubscriptionRequest` data was lost (both were relocated to owning apps in items 2/10 well before this deletion, not carried in `sitesettings` at all by the time it was removed).

**`scuba.sitesettings` retirement plan: fully closed.**
